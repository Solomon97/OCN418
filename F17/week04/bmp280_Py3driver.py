...
class BMP280:
    ...
    def set_filter(self, coef=None):
        ...
        if coef is None:
            ...
            if coef not in filter_coef:
                raise ValueError(
                    'Valid filter coefficient: {0(no filter),2,4,8,16}')
    ...
    def read(self):
        for i in range(1000):
            ...
            if not (status & (1 << 3)):
                ...
                time.sleep(0.001)
        C = [self.bus.read_byte_data(self.address, i)
             for i in range(0xF7, 0xFC + 1)]
        adc_P = ((C[0] << 16) + (C[1] << 8) + C[2]) >> 4
        adc_T = ((C[3] << 16) + (C[4] << 8) + C[5]) >> 4

        var1 = (adc_T / 16384 - self.dig_T1 / 1024) * self.dig_T2
        var2 = (adc_T / 131072 - self.dig_T1 / 8192) * \
            (adc_T / 131072 - self.dig_T1 / 8192) * self.dig_T3
        t_fine = var1 + var2
        ...
        var1 = (self.dig_P3 * var1 * var1 /
                524288 + self.dig_P2 * var1) / 524288
    ...
    def _read_compensation_parameters(self):
        tmp = [self.bus.read_byte_data(self.address, i)
               for i in range(0x88, 0x9F + 1)]
        C = struct.unpack('<HhhHhhhhhhhh',bytes(tmp))
        self.dig_T1 = C[0]
        self.dig_T2 = C[1]
        self.dig_T3 = C[2]
        self.dig_P1 = C[3]
